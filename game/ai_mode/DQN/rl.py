...
class DQN:
    ...
    def learn_and_update_weights_by_replay(self, model_target):
        ...
        targets = rewards + self.gamma * (np.amax(model_target.predict_on_batch(next_states), axis=1)) * (1 - done_list)
    ...
    def train(self, num_episodes=500, can_stop=True):
        ...
        for episode in range(num_episodes):
            ...
            if ((episode > 0) and (episode % self.refresh_target_model_num == 0)):  
                self.model_target.set_weights(self.model.get_weights())
