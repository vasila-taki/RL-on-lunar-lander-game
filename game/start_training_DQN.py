...
from ai_mode.DQN.rl import DQN
...
def run_experiment_for_gamma():
    ...
    for gamma_value in gamma_list:
        ...
        model = DQN(env, lr, gamma_value, epsilon, epsilon_decay, upd_freq, batch_size)
...
def run_experiment_for_batch_size():
    ...
    for batch_size_value in batch_size_list:
        ...
        model = DQN(env, lr, gamma, epsilon, epsilon_decay, upd_freq, batch_size_value)
...
def run_experiment_for_upd_freq():
    ...
    for upd_freq_value in upd_freq_list:
        model = DQN(env, lr, gamma, epsilon, epsilon_decay, upd_freq_value, batch_size)
...
def run_experiment_for_lr():
    ...
    for lr_value in lr_values:
        model = DQN(env, lr_value, gamma, epsilon, epsilon_decay, upd_freq, batch_size)
...
def run_experiment_for_ed():
    ...
    for ed in ed_values:
        ...
        model = DQN(env, lr, gamma, epsilon, ed, upd_freq, batch_size)
...
if __name__ == '__main__':
    ...
    pickle.dump(test_rewards, open(save_dir + "test_rewards_DQN.p", "wb"))
    test_rewards = pickle.load(open(save_dir + "test_rewards_DQN.p", "rb"))

    plot_df2(pd.DataFrame(test_rewards), "Figure 2: Reward for each testing episode_DQN","Reward for each testing episode", "Episode", "Reward")
